...
// ===== VARIABLES GLOBALES DE SAISIE =====
// Definies ici, accessibles via extern dans les autres modules
...
// Etat ecran information
infoScreenState_t infoScreenState = INFO_SCREEN_IDLE;

// Clavier et menus (definis ici provisoirement, a deplacer dans keypad/menus)
key_code_t touche = KEY_NONE;
...
bool startupListActivated = false;
char saisieActive = 0;
char stringSaisie[OLEDBUFLEN] = "azerty";
...
// Email charset
char emailCharSet[] = "abcdefghijklmnopqrstuvwxyz0123456789@.-_";
uint8_t emailCharSetSize = 40;
...
void initStartupList(void)
{
  ...
  #if 0 // TODO: porter vers U8g2 — pushMenu depend du systeme de menus
  pushMenu("-- MENU PRINCIPAL --", m0_Demarrage, M0_ITEM, 0);
  #endif

  startupListActivated = true;
  Serial.println("initStartupList()\\Menu principal active");
  Serial.print("initStartupList()\\currentMenuDepth apres init: ");
  Serial.println(currentMenuDepth);

// SerialPrintMenuStruct(currentMenu);
...
void startListInput(const char* title, const char** itemList, uint8_t numItems, uint8_t initialIndex, unsigned long timeoutMs)
{
  ...
  Serial.print("startListInput()\\Selection dans liste demarree: ");
...
// afficher itemList
  uint8_t i =0;
  Serial.println("startListInput()fin\\Liste des options du menu :");
  for (i=0; i<numItems; i++)
  {
    Serial.print(itemList[i]);
    Serial.print(" - ");
  }
Serial.println("");
...
listInputState_t processListInput(void)
{
  ...
  if (listInputCtx.timeoutDuration > 0 && (millis() - listInputCtx.lastActivity > listInputCtx.timeoutDuration))
  {
    ...
    Serial.println("Selection annulee par timeout");
    #if 0 // TODO: porter vers U8g2
  ...
  if (touche != KEY_NONE)
  {
    ...
    switch (touche)
    {
      case UP:
        ...
        listInputCtx.displayRefresh = true;
        Serial.print("Selection index: ");
        Serial.print(listInputCtx.selectedIndex);
        Serial.print(" - ");
        Serial.println(listInputCtx.itemList[listInputCtx.selectedIndex]);
        break;
        
      case DOWN:
        // Descendre dans la liste
        Serial.println("Down de processListInput"); 
      ...
      case VALIDE:
        ...
        Serial.print("Selection validee: ");
        Serial.print(listInputCtx.selectedIndex);
        Serial.print(" - ");
        Serial.println(listInputCtx.itemList[listInputCtx.selectedIndex]);
        #if 0 // TODO: porter vers U8g2
...
uint8_t finalizeListInput(char* outputItem)
{
  ...
  if (!outputItem)
  {
    Serial.println("Erreur: outputItem NULL");
    return 0xFF;
  }
  ...
  if (listInputCtx.state == LIST_INPUT_COMPLETED)
  {
    ...
    if (listInputCtx.itemList && selectedIndex < listInputCtx.maxItems)
    {
      ...
      Serial.print("Item final selectionne [");
      Serial.print(selectedIndex);
      Serial.print("]: ");
      Serial.println(outputItem);
    }
    else
    {
      strcpy(outputItem, "NONE");
      Serial.println("Erreur: item invalide, retour NONE");
    }
  ...
  else if (listInputCtx.state == LIST_INPUT_CANCELLED)
  {
    ...
    Serial.println("Selection annulee (timeout), retour: NONE");
  ...
  else
  {
    // État inattendu
    strcpy(outputItem, "NONE");
    Serial.print("Erreur: etat inattendu (");
    Serial.print(listInputCtx.state);
    Serial.println("), retour: NONE");
  }
...
{
  listInputCtx.state = LIST_INPUT_CANCELLED;
  Serial.println("Selection dans liste annulee");
  #if 0 // TODO: porter vers U8g2
...
void modifyStringChar(char* str, uint8_t pos, int delta)
{
  ...
  Serial.print("Position string ");
  Serial.print(pos);
  Serial.print(" modifiee: '");
...
void startStringInput(const char* title, const char* initialString, uint8_t maxLength)
{
  ...
  Serial.print("Saisie alphanumérique demarree: ");
  Serial.println(title);
...
stringInputState_t processStringInput(void)
{
  ...
  if (stringInputCtx.timeoutDuration > 0 && (millis() - stringInputCtx.lastActivity > stringInputCtx.timeoutDuration))
  {
    ...
    Serial.println("Saisie alphanumérique annulee par timeout");
    #if 0 // TODO: porter vers U8g2
    OLEDDisplayMessageL8("Timeout             ", false, false);
    #endif
    return stringInputCtx.state;
    ...
    switch (touche)
    {
      case LEFT:
        ...
        if (stringInputCtx.position > 0)
        {
          ...
          stringInputCtx.displayRefresh = true;
          Serial.print("Position curseur string: ");
          Serial.println(stringInputCtx.position);
        }
        break;
      ...
      case VALIDE:
        ...
        Serial.print("Chaine alphanumérique validee: ");
        Serial.println(stringInputCtx.workingString);
        #if 0 // TODO: porter vers U8g2
...
{
  stringInputCtx.state = STRING_INPUT_CANCELLED;
  Serial.println("Saisie alphanumérique annulee");
  #if 0 // TODO: porter vers U8g2
  OLEDDisplayMessageL8("Saisie annulee      ", false, false);
  #endif
...
void startHexInput(const char* title, const char* initialHex, uint8_t maxLen)
{
  ...
  {
    Serial.println("Saisie hexadecimale en cours????");
    return; // Saisie déjà en cours
  }
  ...
  Serial.print(hexInputCtx.title);
  Serial.print(" => Saisie hexa demarree (longueur: ");
  Serial.print(maxLen);
  Serial.println(")");
...
hexInputState_t processHexInput(void)
{
      ...
      (millis() - hexInputCtx.lastActivity > hexInputCtx.timeoutDuration))
  {
    ...
    Serial.println("Saisie hexadecimale annulee par timeout");
    #if 0 // TODO: porter vers U8g2
    OLEDDisplayMessageL8("Timeout             ", false, false);
    #endif
    return hexInputCtx.state;
    ...
    switch (touche)
    {
      case LEFT:
        if (hexInputCtx.position > 0)
        {
          ...
          hexInputCtx.displayRefresh = true;
          Serial.print("Position curseur hex: ");
          Serial.println(hexInputCtx.position);
        }
        break;
      ...
      case VALIDE:
        if (isHexStringValid(hexInputCtx.workingHex, hexInputCtx.maxLength))
        {
          ...
          Serial.println("Chaine hexadecimale validee");
          #if 0 // TODO: porter vers U8g2
        ...
        {
          Serial.println("Chaine hexadecimale invalide");
          #if 0 // TODO: porter vers U8g2
...
void finalizeHexInput(char* outputHex)
{
  if (hexInputCtx.state == HEX_INPUT_COMPLETED)
  {
    ...
    Serial.print("Chaine hexadecimale finale: ");
...
{
  hexInputCtx.state = HEX_INPUT_CANCELLED;
  Serial.println("Saisie hexadecimale annulee");
  #if 0 // TODO: porter vers U8g2
  OLEDDisplayMessageL8("Saisie annulee      ", false, false);
  #endif
...
// Copier le titre (max 20 caractères)
  ...
  hexInputCtx.firstDisplay = true;
//Serial.print(hexInputCtx.title);
//Serial.println("=> Saisie hexadecimale demarree");
  #if 0 // TODO: porter vers U8g2
  OLEDDisplayMessageL8("Modifiez la cle hexa", false, false);
  #endif
}
...
void startTimeInput(const char* title, const char* initialTime)
{
  ...
  Serial.println("Saisie d'heure demarree");
  #if 0 // TODO: porter vers U8g2
...
timeInputState_t processTimeInput(void)
{
  ...
  if (timeInputCtx.timeoutDuration > 0 && (millis() - timeInputCtx.lastActivity > timeInputCtx.timeoutDuration))
  {
    ...
    Serial.println("Saisie heure annulee par timeout");
    #if 0 // TODO: porter vers U8g2
    OLEDDisplayMessageL8("Timeout             ", false, false);
    #endif
    return timeInputCtx.state;
  ...
  if (touche != KEY_NONE)
  {
    ...
    switch (touche)
    {
      case LEFT:
        {
          ...
          if (newPos != timeInputCtx.position)
          {
            ...
            timeInputCtx.displayRefresh = true;
            Serial.print("Position curseur heure: ");
            Serial.println(timeInputCtx.position);
          }
        }
        break;
      ...
      case VALIDE:
        ...
        {
          Serial.println("Heure invalide, validation refusee");
          #if 0 // TODO: porter vers U8g2
...
{
  timeInputCtx.state = TIME_INPUT_CANCELLED;
  Serial.println("cancelTimeInput() Saisie d'heure annulee");
  #if 0 // TODO: porter vers U8g2
  OLEDDisplayMessageL8("Saisie annulee", false, false);  
  #endif
  // Reset du contexte
...
void startDateInput(const char* title, const char* initialDate)
{
  ...
  Serial.println("Saisie de date demarree");
  #if 0 // TODO: porter vers U8g2
...
dateInputState_t processDateInput(void)
{
  ...
  if (dateInputCtx.timeoutDuration > 0 && (millis() - dateInputCtx.lastActivity > dateInputCtx.timeoutDuration))
  {
    ...
    Serial.println("Saisie date annulee par timeout");
    #if 0 // TODO: porter vers U8g2
    OLEDDisplayMessageL8("Timeout", false, false);
    #endif
    return dateInputCtx.state;
  ...
  if (touche != KEY_NONE)
  {
    ...
    switch (touche)
    {
      case LEFT:
        {
          ...
          if (newPos != dateInputCtx.position)
          {
            ...
            dateInputCtx.displayRefresh = true;
            Serial.print("Position curseur date: ");
            Serial.println(dateInputCtx.position);
          }
        }
        break;
      ...
      case VALIDE:
        ...
        {
          Serial.println("Date invalide, validation refusee");
          #if 0 // TODO: porter vers U8g2
...
void finalizeDateInput(char* outputDate)
{
  if (dateInputCtx.state == DATE_INPUT_COMPLETED)
  {
    ...
    Serial.print("Date finale: ");
    Serial.println(outputDate);
  }
}
...
{
  dateInputCtx.state = DATE_INPUT_CANCELLED;
  Serial.println("Saisie de date annulee");
  #if 0 // TODO: porter vers U8g2
  OLEDDisplayMessageL8("Saisie annulee      ", false, false);
  #endif
...
void modifyIPDigit(char *ip, uint8_t pos, int delta) 
{
  ...
  Serial.print("Position IP ");
  Serial.print(pos);
  Serial.print(" modifiee, octet: ");
...
void startIPInput(const char* initialIP)
{
  ...
  Serial.println("Saisie IP demarree");
  #if 0 // TODO: porter vers U8g2
...
ipInputState_t processIPInput(void)
{
  ...
  if (ipInputCtx.timeoutDuration > 0 && (millis() - ipInputCtx.lastActivity > ipInputCtx.timeoutDuration))
  {
    ...
    Serial.println("Saisie IP annulee par timeout");
    #if 0 // TODO: porter vers U8g2
    OLEDDisplayMessageL8("Timeout             ", false, false);
    #endif
    return ipInputCtx.state;
      ...
      case LEFT:
        {
          ...
          if (newPos != ipInputCtx.position)
          {
            ...
            ipInputCtx.displayRefresh = true;
            Serial.print("Position curseur IP: ");
            Serial.println(ipInputCtx.position);
          }
        }
        break;
...
void finalizeIPInput(char* outputIP)
{
  if (ipInputCtx.state == IP_INPUT_COMPLETED)
  {
    ...
    Serial.print("IP finale: ");
    Serial.println(outputIP);
  }
}
...
{
  ipInputCtx.state = IP_INPUT_CANCELLED;
  Serial.println("Saisie IP annulee");
  #if 0 // TODO: porter vers U8g2
  OLEDDisplayMessageL8("Saisie annulee      ", false, false);
  #endif
...
void startEmailInput(const char* initialEmail)
{
  ...
  Serial.println("Saisie email demarree");
  #if 0 // TODO: porter vers U8g2
...
emailInputState_t processEmailInput(void)
{
  ...
  if (emailInputCtx.timeoutDuration > 0 && (millis() - emailInputCtx.lastActivity > emailInputCtx.timeoutDuration))
  {
    ...
    Serial.println("Saisie email annulee par timeout");
    #if 0 // TODO: porter vers U8g2
    OLEDDisplayMessageL8("Timeout             ", false, false);
    #endif
    return emailInputCtx.state;
    ...
    switch (touche)
    {
      case LEFT:
        ...
        if (emailInputCtx.position > 0)
        {
          ...
          emailInputCtx.displayRefresh = true;
          Serial.print("Position curseur email: ");
          Serial.println(emailInputCtx.position);
        }
        break;
...
void finalizeEmailInput(char* outputEmail)
{
  if (emailInputCtx.state == EMAIL_INPUT_COMPLETED)
  {
    ...
    Serial.print("Email final: ");
    Serial.println(outputEmail);
  }
}
...
{
  emailInputCtx.state = EMAIL_INPUT_CANCELLED;
  Serial.println("Saisie email annulee");
  #if 0 // TODO: porter vers U8g2
  OLEDDisplayMessageL8("Saisie annulee      ", false, false);
  #endif
